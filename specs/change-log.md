...
- `chat/page.tsx`: added `nextCursor` state — captured from the initial REST load and updated after each "load older" fetch. The button only renders when `nextCursor` is non-null (i.e., there are older messages the backend hasn't sent yet).
- `chat/page.tsx`: added `handleLoadOlder()` — fetches the previous page using `?cursor=<id>`, merges results via the existing `mergeMessages` helper (deduplicates + sorts chronologically), and updates `nextCursor` for the next page.
- `chat/page.tsx`: added scroll-position preservation — snapshots `listRef.current.scrollHeight` before the state update and restores `scrollTop` to `newScrollHeight - savedScrollHeight` after React re-renders, so the viewport stays at the same message instead of jumping to the bottom.
...
- Added "Tải thêm tin nhắn cũ" button with `ChevronUp` icon at the top of the message list; shows a `Loader2` spinner while loading.
...
- Open the Chat tab — only the newest 50 messages load; a "Tải thêm tin nhắn cũ" button appears at the top.
- Click the button — the previous page of messages prepends to the list and the viewport stays at the same position (does not jump to bottom).
...
## 2026-05-30 (192) — Fix isMe detection in chat using user ID (S1)
...
- `chat/page.tsx`: changed `isMe` from `senderName(msg) === me.name` to `msg.member.userId === me.id` — eliminates the false-positive where two users with identical display names would see each other's messages styled as their own.
...
- Send a message from account A — account B should see it on the left (other person's message), not the right (own message).
- Previously, account B would render A's message as if it were their own because the names matched.

---

## 2026-05-30 (191) — Load lastReminderAt from server on balances page mount (M1)
...
**How to test:**
- As organizer, open the Balances tab and send a reminder to a debtor — the button should switch to cooldown state with a timestamp.
- Reload the page — the cooldown state and timestamp should still be visible (previously they would reset to "Nhắc nhở").
- Wait 24 hours (or temporarily lower `COOLDOWN_MS`) — the button should return to active state.

---

## 2026-05-30 (190) — Round display amounts to nearest 1,000 ₫ (F5)

**Tasks completed:**

- Fix `formatVND` in `balances/page.tsx`, `admin/page.tsx`, and `vnd()` in `pdf.generator.ts`: changed `Math.round(amount)` → `Math.round(amount / 1000) * 1000` so displayed VND amounts are rounded to the nearest 1,000 ₫ as required by spec §5.4.
...
**How to test:**
- Create an expense with an amount not divisible by 1,000 (e.g. 15,500 ₫ split among 3 people → 5,167 ₫ each).
- Open the Balances tab — amounts should display rounded to the nearest 1,000 ₫ (e.g. 5,000 ₫ not 5,167 ₫).
- Open the Admin dashboard and check the "Tổng VND theo dõi" stat — any fractional amount should also be rounded.
- Export a PDF and verify the expense/balance amounts are rounded to 1,000 ₫ increments.
...
- `apps/web/lib/hooks/use-auth.ts` _(new)_: `useLogin`, `useRegister`, `useForgotPassword`, `useLogout` — all `useMutation` wrappers over `api.post`.
...
- `apps/api/src/messages/messages.gateway.ts` — `handleSendMessage` now uses `socket.to(room).emit('newMessage', message)` instead of `this.server.to(room).emit(...)`. `socket.to()` broadcasts to all sockets in the room _except_ the sender; the sender receives the message only via the acknowledgment return value, preventing duplicate display on the client.
...
- `apps/api/src/auth/guards/jwt-auth.guard.ts` _(new)_: `JwtAuthGuard` — reads `access_token` HttpOnly cookie, skips if `@Public()`, validates JWT with `JWT_SECRET`, attaches payload to `request.user`; throws 401 if missing or expired.
- `apps/api/src/auth/guards/roles.guard.ts` _(new)_: `RolesGuard` — reads `@Roles()` metadata; if present, checks `request.user.role` is in the allowed list; throws 403 otherwise.
